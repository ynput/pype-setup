...
class SystemTrayIcon(QtWidgets.QSystemTrayIcon):
    ...
    def __init__(self, icon, parent=None):
        ...
        self.menu.setProperty('menu', 'on')
        # label = QtWidgets.QLabel('Zetcode', self.menu)
        # self.main_layout.addWidget(label)
        self.menu.setStyleSheet(textwrap.dedent('''
                                            QWidget {
        ...
        for key, value in menu_layout_dict.items():
            if value == 'action':
                ...
                # insAvPl_Action = self.menu.addAction(key)
                run = QtWidgets.QAction(QtGui.QIcon('exit24.png'), 'Exit', self)
                widget = QtWidgets.QWidget(run)
                widget.addAction(run)
                self.main_layout.addWidget(widget)

                # qlabel = QtWidgets.QLabel(key, self.menu)
                # self.main_layout.addWidget(qlabel)
            else:
                menu = QtWidgets.QMenu(self.menu)
                ...
                self.main_layout.addWidget(avalon_plugins)
