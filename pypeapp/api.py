...
class Api(object):
    ...
    def __init__(self):
        self._log = Logger().get_logger('api')
        pass
    ...
    def execute(self,
        ...
        """ Execute command as process.
            ...
            :rtype: int
        """
        self._log.info("Executing '%s'.." % " ".join(args))
        ...
        while True:
            ...
            if line != '':
                if not silent:
                    line_test = False
                    for funct, test_string in self._log_levels.items():
                        if test_string in line:
                            getattr(self._log, funct)(
                                "Exe: {}".format(
                                    line[:-2]
                                ).replace(
                                    test_string,
                            ...
                            line_test = True
                            break
                    for test_string in self._log_levels_debug:
        ...
        self._log.info("Execution is finishing up ...")
